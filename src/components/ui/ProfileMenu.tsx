...
export default function ProfileMenu() {
    ...
    return (
        <>
            <div onClick={toggleOpenMenu} className="text-center cursor-pointer duration-300 ease-linear relative rounded-full p-2 flex items-center bg-secondary">
                <div className="rounded-full bg-primary w-8 h-8 flex-none flex flex-col items-center justify-center font-bold text-sm">
                    <span className="block">N</span>
                </div>

                <div className={`${openMenu ? "w-fit p-2 ml-2" : "w-0"} overflow-hidden duration-300 text-sm font-bold ease-linear bg-primary rounded-full`}>
                    <span className="w-fit flex-none text-nowrap">Norman Bassey</span>
                </div>
                ...
                <div className={`${openMenu ? "-right-20 " : "-right-96 lg:-right-[600px]"} absolute top-[4.3rem] duration-300 ease-in-out w-40 bg-primary/60 bg-blur dark:bg-white/60 rounded-xl p-4 h-fit flex flex-col gap-4 shadow-md`}>
                    <div className="divide-y-2 divide-light dark:divide-primary flex flex-col space-y-4">
                        ...
                        <Link className="pt-2 hover:text-secondary dark:text-primary duration-300 ease-linear" href="/auth/reset-password">Settings</Link>
                    </div>
