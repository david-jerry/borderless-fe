...
export default function ResendEmailForm() {
    ...
    const submitForm = async () => {
        const res = await auth!.resendVerificationEmail(emailInput!)
        console.log(res);
        ...
        <>
            {
                !sent ?
                    <form action={submitForm} className="pt-8 max-w-xs lg:max-w-md w-full space-y-6">
                        ...
                        <button type="submit" disabled={auth?.isLoading || error ? true : false} className={`rounded-xl hover:scale-105 duration-300 ease-linear hover:shadow-lg w-full py-2 px-6 ${error ? "bg-gray-300 text-gray-900" : "bg-primary dark:bg-secondary text-light dark:text-dark"}`}><span className={`${auth?.isLoading ? "animate-ping" : "animate-none"}`}>{auth?.isLoading ? "Submitting..." : "Submit"}</span></button>
                    </form>
                    :
                    <div className="mt-8 max-w-xs lg:max-w-md w-full gap-6 text-center flex flex-row items-center bg-green-300 text-green-700 rounded-xl p-6">
                        ...
                        <p className="font-semibold text-sm flex-grow">
                            We have sent a confirmation email to you, please confirm and verify your email account.
                        </p>
